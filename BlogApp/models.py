...
class contact_us_data(db.Model):
  id = db.Column(db.Integer(), primary_key = True)
...
class products(db.Model):
  ...
  order_link = db.Column(db.String(), nullable = False)
  colour = db.Column(db.String(), default=False, nullable = False)
  sizes = db.Column(db.String(), default=False, nullable = False)
...
def insert_dummy_data(db):
  pd1 = products(name="Shirt", brand='Adidas', description="A shirt", price=100, order_link='https://forms.gle/hY8nHBJJKxxi8QnH6', colour='bg-red-400', sizes='S,M,L,XL')
  pd2 = products(name="Pants", brand='Nike', description="A pair of pants", price=200)
  pd3 = products(name="Shoes", brand='New Balance', description="Fam locavore kickstarter distillery. Mixtape chillwave tumeric sriracha taximy chia microdosing tilde DIY. XOXO fam indxgo juiceramps cornhole raw denim forage brooklyn. Everyday carry +1 seitan poutine tumeric. Gastropub blue bottle austin listicle pour-over, neutra jean shorts keytar banjo tattooed umami cardigan.", price=300)
  db.session.add(pd1)
  db.session.add(pd2)
  db.session.add(pd3)
  db.session.commit()
  
  a1 = announcements(title="Announcement 1", category='giveaway', description="This is the first announcement")
  a2 = announcements(title="Announcement 2", category='welp', description="This is the second announcement")
  db.session.add(a1)
  db.session.add(a2)
  db.session.commit()
